...
public class ContentSafetyFilter {
    ...
    public boolean passes(String text) {
        ...
        for (String keyword : blocklist) {
            if (lower.contains(keyword)) {
                log.warn("SAFETY_FILTER: Discarding content matching blocked keyword '{}'", keyword);
                return false;
            }
        }
        return true;
    ...
    private List<String> loadBlocklist() {
        try (InputStream is = getClass().getClassLoader().getResourceAsStream("safety-blocklist.txt")) {
            if (is == null) {
                log.warn("SAFETY_FILTER: safety-blocklist.txt not found on classpath; using empty blocklist");
            ...
            log.error("SAFETY_FILTER: Failed to load blocklist", e);
            return Collections.emptyList();
        }
    }
}
